...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_Cauchy_modulated_RL/")
...
N=2**19
sigcor = cosine_taper(sce)
fftSigi=1/Nt*np.fft.fftshift(np.fft.fft(sigcor,n=N))
fftSigf=1/Nt*np.fft.fftshift(np.fft.fft(Vfilm[Ntfig,:]*cm[0],n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
plt.figure()
plt.plot(np.abs(freq),np.abs(fftSigi),linewidth=3, label='Source')
plt.plot(np.abs(freq),np.abs(fftSigf),linewidth=3, label='Transmitted wave')
plt.xlim([0,800])
plt.ylim([0,0.11])
plt.xlabel(r'$f$ (Hz)',fontsize=16)
# plt.ylabel(r'$\overline{S}$',fontsize=16)

plt.ylabel(r'$\hat{v}$',fontsize=16)
