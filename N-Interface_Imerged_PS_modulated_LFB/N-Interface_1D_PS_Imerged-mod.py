...
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_LFB/")
...
rhoT,CT=homogenize.timeProp(Nt, dt2p, t, rho, cm, frontiere, modulation)
...
for ti in range(int(Nt/pFilm)):
    energyV[ti]=dx/2*np.sum(rhoT[ti*pFilm]*VfilmGS[ti]*VfilmGS[ti])+1/2*np.sum(1/rhoT[ti*pFilm]/CT[ti*pFilm]/CT[ti*pFilm]*SfilmGS[ti]*SfilmGS[ti])
    energyI[ti]=1/2*(M(ti)*immergedinterfaces.VM)
plt.figure()
plt.plot(t[:-2:pFilm],energyV)
plt.imshow([CT[::pFilm]], cmap='gray', extent=[t[0], t[-2], 0,np.max(energyV)], aspect='auto', alpha=0.35)
# timeN_mod=timef-time0
# print(timeN_mod)  
# ################ STRANG SPLITTING ####################################
# plt.grid()
...
def updateComp(frame):
    ...
    plt.imshow([np.linspace(rhoT[frame],rhoT[frame],Nx)], cmap='gray',vmax=max(rhoT),vmin=min(rhoT), extent=[xmin, xmax, -0.0015,0.0015], aspect='auto', alpha=0.35)
    # plt.colorbar()
    # plt.plot(X, VfilmH[frame,:])
    plt.xlabel("x (m)")
    plt.ylabel("v (m/s)")
...
# plt.figure()
# plt.plot(Vfilm[700,:],"*")
# plt.plot(VfilmGS[700,:],"--")film[400:900,:])*1000))
...
frqvG,wavvG,FFKG=specfkR(VfilmGS[::,::], dx, dt2p*pFilm, int(Ntp/pFilm)+1, int((Nx)))
# plt.figure()
