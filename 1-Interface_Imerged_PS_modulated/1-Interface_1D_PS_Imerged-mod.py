...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_PS_modulated/")
...
import time
time0=time.time()
Res,Vfilm,Sfilm,CS=scheme1D.FD_sourceVII(U0,Nt,Nx,K,rhox,Celx,dt,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm,rCSVM="yes")#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
...
# plt.plot(Vfilm[::10,int(x_0R)])
# N=512
# fftSig=np.fft.fft(Vfilm[::,int(x_0R)],n=N)
# freq = np.fft.fftfreq(N)
# plt.figure()
# plt.plot(freq[:int(N/2)],np.abs(fftSig[:int(N/2)]))
