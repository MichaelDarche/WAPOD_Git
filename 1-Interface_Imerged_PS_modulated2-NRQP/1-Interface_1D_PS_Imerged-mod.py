...
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_PS_modulated2-NRQP/")
...
errQP=np.zeros([Ns,Nm])
# nameSource="Source"+".txt"
# nameSourceR="SourceRev"+".txt"
for nsource in range(2,Ns+1):
...
#%
        ...
        errQP[nsource-1,nmodu]=np.sum(diff**2)
...
for i in range(nsource+1):
    for j in range(nmodu+1):
        plt.scatter(fmod[j],errQP[i,j],color=colorplot[i])
...
for i in range(nsource):
    # f=fsou[i]
    # fmodm=fmod/f
    plt.plot(fmod,np.sqrt(err[i])*30)
plt.xlabel(r'$f_m$ (Hz)')
plt.ylabel(r'$\varepsilon_v=\sqrt{\sum(v_d-v_r)^2}$ (m/s)')

plt.figure()
plt.plot(fmod,30*errQP[0,:],label=r'$f_s=30$ Hz',linewidth=3)
plt.plot(fmod,50*errQP[1,:],'--',label=r'$f_s=50$ Hz',linewidth=3)
plt.xlabel(r'$f_m$ (Hz)',fontsize=16)
plt.ylabel(r'$f_s\varepsilon_v$ (m)',fontsize=16)
