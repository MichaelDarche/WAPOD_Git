...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/2-Interface_Imerged_Cauchy_modulated_Key/")
...
import time
time0=time.time()
Res,Vfilm,Sfilm,CSVM,traces=scheme1D.FD_cauchyII(U0,Nt,Nx,K,rhox,Celx,dt,dx,configuration,mat,frontiere,modulation,pFilm,rCSVM="yes",rtraces="yes")
...
# #animation.save("FilmCauchyCrypto.mp4", writer='ffmpeg', fps=30)
...
sigcor = cosine_taper(Vfilm[::,int(400)])
plt.plot(Vfilm[::,int(400)])
# plt.plot(sigcor)
fftSig1=np.fft.fftshift(np.fft.fft(sigcor,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
# plt.figure()
# plt.plot(np.abs(freq),np.abs(fftSig1))

sigcor2 = cosine_taper(Vfilm[::,int(500)])
plt.plot(Vfilm[::,int(500)])
# plt.plot(sigcor2)
fftSig2=np.fft.fftshift(np.fft.fft(sigcor2,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
# plt.figure()
# plt.plot(np.abs(freq),np.abs(fftSig2))

sigcor3 = cosine_taper(Vfilm[::,int(600)])
plt.plot(Vfilm[::,int(600)])
# plt.plot(sigcor3)
fftSig3=np.fft.fftshift(np.fft.fft(sigcor3,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
# plt.figure()
# plt.plot(np.abs(freq),np.abs(fftSig3))

plt.figure()
plt.plot(np.abs(freq),np.abs(fftSig1),label="Source")
plt.plot(np.abs(freq),np.abs(fftSig2),label="1 Interface")
plt.plot(np.abs(freq),np.abs(fftSig3),label="2 Interface")
plt.xlabel("freqs",fontsize=20)
