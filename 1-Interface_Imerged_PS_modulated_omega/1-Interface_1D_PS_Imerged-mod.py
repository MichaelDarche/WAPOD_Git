...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_PS_modulated_omega/")
...
N=Nt
sigcor = cosine_taper(Vfilm[::,int(x_0R)])
plt.figure()
...
FFTMap=np.zeros([Nt,Nx])
for n in range(Nx):
    sigcor = cosine_taper(Vfilm[::,n])
    FFTMap[:,n]=np.fft.fftshift(np.fft.fft(sigcor,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
plt.figure()
...
plt.figure()
plt.plot(freq,np.abs(FFTMap[:,500]))
plt.plot(freq,np.abs(FFTMap[:,510]))
