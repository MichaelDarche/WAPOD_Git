...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_dphahomo/")
#os.chdir("/home/darche/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_dpha/")
import numpy as np
...
# # Cree l'animation en certains points
...
for i in range(alpha.size-1):
    if modul.fonctionModK(pFilm*dt2p*2200,alpha[i],modulation,frontiere,i)<0.:
        plt.axvline(x=alpha[i],color='red')#,linestyle='--')
    else:
        plt.axvline(x=alpha[i],color='lime')#,linestyle='--')
plt.plot(X, Vfilm[2200,:],'o')#, label=f'Frame {frame}')
# plt.plot(X, VfilmSS[frame,:],'--')
# plt.plot(X, VfilmGS[frame,:],"-")#, label=f'Frame {frame}')
# # plt.plot(X, VfilmSS[frame,:]-Vfilm[frame,:],"-")#, label=f'Frame {frame}')
# #plt.plot(X, VfilmSSC[frame,:]-Vfilm[frame,:],"-")
...
skipx=10
skipt=12
frqv,wavv,FFK=specfk(Vfilm[::skipt,::skipx], dx*skipx, dt2p*skipt, Ntp//skipt+1, Nx//skipx+1)
plt.figure()
plt.pcolor(wavv,frqv,np.log(FFK))#,vmin=-3, vmax = 2)
# plt.ylim([0,200])
