...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_Cauchy_KeyFile")
...
vectTime,data,sR,sW,nsamp=wttxt.wave2np("Toum")

# vectTime=np.arange(0,0.01,1/44100)
# SceTemp=np.zeros(np.size(vectTime))
# for i in range(len(vectTime)):
#     SceTemp[i]=source.jkps_t(vectTime[i],500)
...
import time
time0=time.time()
Res,Vfilm,Sfilm=scheme1D.FD_sourceVII(U0,Nt,Nx,K,rhox,Celx,dt,dx,x_0,sigcor,configuration,mat,frontiere,modulation,pFilm,printT=100)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
...
modulatedSound=wttxt.np2wav("ToumModulated",intV,sR,sW,nsamp-1)

sigcorM = cosine_taper0(Vmodul)
fftSigM=1/Nt*np.fft.fftshift(np.fft.fft(sigcorM,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
plt.figure()
plt.plot(np.abs(freq),np.abs(fftSigM),linewidth=3)
# plt.xlim([0,300])
# plt.ylim([0,0.25])
plt.xlabel(r'$f$ (Hz)',fontsize=16)
...
import time
time0=time.time()
ResR,VfilmR,SfilmR=scheme1D.FD_sourceVIIKey(U0,Nt-1,Nx,K,rhox,Celx,-dt,dx,x_r,-2*np.flip(Vfilm[:,Nxr])*cm[0],configuration,mat,frontiere,modulationR,pFilm,printT=100)
...
modulatedRSound=wttxt.np2wav("ToumBack",intR,sR,sW,nsamp-1)

sigcorB = cosine_taper0(Vback)
fftSigB=1/Nt*np.fft.fftshift(np.fft.fft(sigcorB,n=N))
freq = np.fft.fftshift(np.fft.fftfreq(N,d=dt))
plt.figure()
plt.plot(np.abs(freq),np.abs(fftSigB),linewidth=3)
# plt.xlim([0,300])
# plt.ylim([0,0.25])
plt.xlabel(r'$f$ (Hz)',fontsize=16)

plt.figure()
plt.plot(Vfilm[1:,int(x_0/dx)]/2)
plt.plot(np.flip(VfilmR[:,int(x_0/dx)]))
plt.figure()
plt.plot(Vfilm[1:,Nxr])
plt.plot(np.flip(VfilmR[:,Nxr])/2)
