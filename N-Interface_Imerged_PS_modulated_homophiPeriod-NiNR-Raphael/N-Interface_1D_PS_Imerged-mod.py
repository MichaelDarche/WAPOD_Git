...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_homophiPeriod-NiNR-Raphael/")
...
def updateComp(frame):
    ...
    for i in range(alpha.size-1):
        if frontiere["Contact_"+str(i)]!="parfait":
            if modul.fonctionModMP(Ntfig*dt2p,alpha[i],frontiere,i)<0.:
                plt.axvline(x=alpha[i],color='palegreen',alpha=-modul.fonctionModMP(Ntfig*dt2p,alpha[i],frontiere,i)+0.1)#,linestyle='--')
            elif modul.fonctionModMP(Ntfig*dt2p,alpha[i],frontiere,i)>0.:
                plt.axvline(x=alpha[i],color='plum',alpha=modul.fonctionModMP(Ntfig*dt2p,alpha[i],frontiere,i)+0.1)#,linestyle='--')
            else:
...
plt.show()
# nomAnim="Film_1I_"+sourcef["Frequence"]+modulation["Freq"]+"Hz.mp4"
...
plt.tight_layout()
namefig="Figures/PRSA_U_MK-NiR_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
plt.savefig(str(namefig), format='pdf')
namefig="Figures/PRSA_U_MK-NiR_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
plt.savefig(str(namefig), format='png')


plt.figure()
plt.plot(tp[:-1],Ufilm[:,1800],label=r'$U_{left}$',linewidth=3)
plt.plot(tp[:-1],Ufilm[:,2200],label=r'$U_{right}$',linewidth=3)
...
plt.legend(fontsize=16)#loc="lower right",
plt.tight_layout()
namefig="Figures/PRSAzoom_U_MK-NiR_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
plt.savefig(str(namefig), format='pdf')
namefig="Figures/PRSAzoom_U_MK-NiR_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
plt.savefig(str(namefig), format='png')
