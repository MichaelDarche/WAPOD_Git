...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_Cauchy_modulated")
...
plt.figure()
plt.plot(2*np.pi*np.abs(freq),np.abs(fftSig),linewidth=3)
plt.axvline()
plt.xlim([0,2*np.pi*250])
plt.ylim([0,0.25])
plt.xlabel(r'$\omega$ (rad/s)',fontsize=16)
# plt.ylabel(r'$\overline{S}$',fontsize=16)
plt.ylabel(r'$\hat{S}$',fontsize=16)
plt.axvline(x=2*np.pi*fs,color='red',linestyle='--')
plt.tight_layout()
...
mod.append(frM)
diffe=energyM[-1]-energyM[0]
diffener.append(diffe)
# deltaener150=[]
# for i in range(9):
#     deltaener150.append(ener150[i]-ener150[0])

# plt.figure()
# plt.scatter(mod,deltaener50,marker='s',label='$f_m=50$ Hz')
# plt.scatter(mod,deltaener100,marker='o',label='$f_m=100$ Hz')
# plt.scatter(mod,deltaener150,marker='*',label='$f_m=150$ Hz')
# plt.xlabel(r'$\varepsilon_{C,M}$',fontsize=16)
# plt.ylabel(r'$\Delta\mathcal{E}$ (J)',fontsize=16)
