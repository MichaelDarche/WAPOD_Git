...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interfaces_direct/")
...
cmH,rhoH,CFLH=homogenize.hPropBicouche(cm[0],rho[0],cm[1],rho[1],l1,l2,CFL)
sourceH["Force"]=str(rhox[int(x_0/dx)]/rhoH)
matH["Rho_0 "]=str(rhoH)
matH["Cel_0 "]=str(cmH)
sourceH["CFL"]=str(CFLH)
sce=np.zeros(Nt+1)
for i in range(Nt+1):
...
# Cree l'animation
animationH = FuncAnimation(fig, updateComp, frames=total_frames, interval=100)
plt.show()
