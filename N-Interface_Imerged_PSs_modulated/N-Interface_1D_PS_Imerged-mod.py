...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PSs_modulated/")
...
import time
time0=time.time()
Res,Vfilm,Sfilm=scheme1D.FD_sourceSII(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
timef=time.time()
...
# K=4
# timef=time.time()
ResH,VfilmH,SfilmH=scheme1D.FD_sourceSIIHomoCara(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
# maxV=np.max([np.max(Vfilm),-np.min(Vfilm)])
# timeN_mod=timef-time0
# print(timeN_mod)  
# Fonction pour mettre à jour le contenu du graphique à chaque image
...
# fig, ax = plt.subplots()
# timef=time.time()
ResGS,VfilmGS,SfilmGS=scheme1D.FD_sourceSIIHomoGS(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)
# timeN_mod=timef-time0
# print(timeN_mod)  
# ################ STRANG SPLITTING ####################################
# plt.grid()
...
# fig, ax = plt.subplots()
# timef=time.time()
ResSS,VfilmSS,SfilmSS=scheme1D.FD_sourceSIIHomoSS(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)
# timeN_mod=timef-time0
# print(timeN_mod)  
# # ###################### NO MODULATION #########25########################
# time0=time.time()
...
frqv,wavv,FFK=specfk(Vfilm, dx, dt2p, Ntp, Nx)
plt.figure()
plt.pcolor(wavv,frqv,np.log(FFK))
plt.ylim([0,200])
