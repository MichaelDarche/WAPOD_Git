...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_homophi_ImpMatch/")
...
eta0=2*np.pi*fs/c_0*h*Nip
eta1=2*np.pi*(fs+fm)/c_0*h*Nip
...
import time
time0=time.time()
Res,Vfilm,Sfilm=scheme1D.FD_sourceVII(U0,Ntp,Nx,K,rhox,Celx,dt2,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
timef=time.time()
...
def updateComp(frame):
    ...
    plt.imshow([rhox], cmap='gray', extent=[xmin, xmax, -np.max(U),np.max(U)], aspect='auto', alpha=0.35)
    plt.colorbar()
    # plt.plot(X, VfilmH[frame,:])
