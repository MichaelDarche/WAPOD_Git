...
def FD_cauchyII(U0,Nt,Nx,K,rhox,cmx,dt,dx,config,mat,frontiere,film):
    ...
    for ts in range(Nt):
        ...
        Un = np.zeros([2,Nx])
        for xstep in range(int(K/2),Nx-int(K/2)):
            if K/2*dx<=np.abs(xstep*dx+xmin-alpha[interS]):
                Us=np.zeros([2,1])
                # rhos=rhox[xstep]
                # cms=cmx[xstep]
                # A=inputReading.comA(rhos,cms)
                ...
                if K/2*dx>np.abs(xstep*dx+xmin-alpha[interS]) and xstep*dx+xmin<=alpha[interS]:
                    Us=np.zeros([2,1])
                    # rhos=rhox[xstep]
                    # cms=cmx[xstep]
                    # A=inputReading.comA(rhos,cms)
...
def FD_sourceV(U0,Nt,Nx,K,rho,cm,dt,dx,x_0,sce_t,film):
    ...
    for ts in range(Nt):
        ...
        for xstep in range(int(K/2),Nx-int(K/2)):
            Us=np.zeros([2])
            #A=inputReading.comA(rho[xstep],cm[xstep])
            for s in range(K+1):
                cs=csTot[s,xstep]
                sumUs=np.dot(cs,U[:,xstep+s-int(K/2)])
                Us=Us+sumUs
