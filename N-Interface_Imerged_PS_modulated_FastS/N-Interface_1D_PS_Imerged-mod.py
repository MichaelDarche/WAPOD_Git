...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_FastS/")
...
################ Parameters of the simulation   
Nmat,rho,cm,S1=inputReading.materialWillis(matFS)
alphaFS=inputReading.frontiere(frontiereFS)
rhoxFS,CelxFS,S1xFS=inputReading.affectMaterialsWillis(alphaFS,rho,cm,S1,xmin,xmax,Nx,dx)
dtFS=scheme1D.timeStep(CFL,dx,cm)/2
# lambdaOnde=cm/fs
# xsmin=cm*tshift-lambdaOnde
# xsmax=cm*tshift
...
coeffsce=rhox[int(x_0/dx)]/rhoxFS[int(x_0/dx)]

ResFS,VfilmFS,SfilmFS=scheme1D.FD_sourceVWillis(U0,2*Ntp,Nx,K,rhoxFS,CelxFS,S1xFS,dtFS,dx,x_0,coeffsce*sceFS,2)
...
# Cree l'animation
animationComp = FuncAnimation(fig, updateComp, frames=total_frames, interval=0.01)
plt.show()
#nomAnim="Film_MicroHFS_"+sourcef["Frequence"]+frontiere["Freq_0"]+"Hz.mp4"
...
def updateComp(frame):
    ...
    sm = mpl.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    plt.scatter(X[::1],Vfilm[frame,::1],marker=r'$\bigcirc$',label=r'$U_h$',color='red',s=30)
    # plt.plot(X[::1],Ufilm[Ntfig,::1],linestyle=None,marker='o',color='white',markersize=2)
    plt.plot(X,V[frame,:],linewidth=3,label=r'$U_0$')
...
ffg.sismoNorm(X, tp, VfilmFS,10,pFilm,Nt=Ntsismo,norm=maxVt)
...
plt.plot(X,UfilmFS[Ntfig,:],linewidth=3,label=r'$U_{fs}$')
...
plt.plot(X,VfilmFS[Ntfig,:],linewidth=3,label=r'$U_{fs}$')
# plt.axvline(x=alpha[0],color='black')
plt.xlabel(r'$X$ (m)',fontsize=16)
...
if tau!=0:
    ...
    ffg.dispFK_tperiodic2(Ufilm[:Ntfig,:],dt2p,dx,fmlist[0],kmax/2)
    for i in range(NE):
