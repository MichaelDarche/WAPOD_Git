...
def functCauchyK(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    ...
    def fct(y,t):
        ...
        Be=Ben*(1+modulateP.fonctionModB(t-Tinit,alpha[0],modulation,frontiere,0))
        Cprime=1/Kn*modulateP.fonctionModKDer(t-Tinit,alpha[0],modulation,frontiere,0)
        fac=1+rc1/rc0+rc1*(Cprime+Be)
...
def functSourceK(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    ...
    def fct(y,t):
        ...
        Be=Ben*(1+modulateP.fonctionModB(t,alpha[0],modulation,frontiere,0))
...
def functCauchyM(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    ...
    def fct(z,t):
        ...
        Ze=Zen*(1+modulateP.fonctionModZ(t-Tinit,alpha[0],modulation,frontiere,0))
        Mprime=Mn*modulateP.fonctionModMDer(t-Tinit,alpha[0],modulation,frontiere,0)
        fac=1+rc0/rc1+(Mprime+Ze)/rc1
...
def functSourceM(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    ...
    def fct(z,t):
        ...
        Ze=Zen*(1+modulateP.fonctionModZ(t,alpha[0],modulation,frontiere,0))
        Mprime=Mn*modulateP.fonctionModMDer(t,alpha[0],modulation,frontiere,0)
        fac=1+rc0/rc1+(Mprime+Ze)/rc1
        return -rc1/M*(U+fac*z)
    return fct

def functCauchy0B(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    Nmat,rho,cm=inputReading.material(mat)
    alpha=inputReading.frontiere(frontiere)
    namorK = "beta_"+str(0)
    if frontiere.get(namorK)==None:
    ...
    def fct(t):
        ...
        S=KteS*rho[0]*1*sce
        Be=Ben*(1+modulateP.fonctionModB(t-Tinit,alpha[0],modulation,frontiere,0))
        fac=1+rc1/rc0+rc1*(Be)
        return 2*invRiemannR(U, S, rho[0], cm[0])/fac
    return fct

def functCauchy0Z(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    Nmat,rho,cm=inputReading.material(mat)
    alpha=inputReading.frontiere(frontiere)
    rc0=rho[0]*cm[0]
    rc1=rho[1]*cm[1]
    ...
    def fct(t):
        ...
        S=KteS*rho[0]*1*sce
        Ze=Zen*(1+modulateP.fonctionModZ(t-Tinit,alpha[0],modulation,frontiere,0))
        fac=1+rc0/rc1+(Ze)/rc1
        return -2*rc0*invRiemannR(U, S, rho[0], cm[0])/fac
    return fct

def functSource0B(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    Nmat,rho,cm=inputReading.material(mat)
    alpha=inputReading.frontiere(frontiere)
    namorK = "beta_"+str(0)
    if frontiere.get(namorK)==None:
...
def functSource0Z(mat,modulation,frontiere,sourcef,KteV=1,KteS=-1):
    Nmat,rho,cm=inputReading.material(mat)
    alpha=inputReading.frontiere(frontiere)
    namorM = "zeta_"+str(0)
    if frontiere.get(namorM)==None:
...
def preProcessSMM(modulation, mat,frontiere,sourcef,dt,Nt,NOde):
    ...
    if Kn != 0 and Mn==0:
        ...
        tRK=np.linspace(Tinit,Tinit+Nt*dt,Nt+1)
        TabUp=np.zeros(Nt+1)
        namorM = "zeta_"+str(0)
        if frontiere.get(namorM)==None:
            etaM=0.
            ...
            etaM= float(frontiere[namorM])
        namorC = "beta_"+str(0)
        if frontiere.get(namorC)==None:
            etaC=0.
        else:
            etaC= float(frontiere[namorC])
        if etaM!=0:
            Vstart=0
            if sourcef['Forme'] == 'Cauchy':
                fctC=functCauchy0Z(mat,modulation,frontiere,sourcef)
                TabY=fctC
            if sourcef['Forme'] == 'point source':
                fctC=functSource0Z(mat,modulation,frontiere,sourcef)
                TabY=fctC
        if etaC!=0:
            Vstart=0
            if sourcef['Forme'] == 'Cauchy':
                fctC=functCauchy0B(mat,modulation,frontiere,sourcef)
            if sourcef['Forme'] == 'point source':
                fctC=functSource0B(mat,modulation,frontiere,sourcef)
        # else:
        #     TabY=intRK4(functSource, Vstart, tRK);
        for I in range(Nt+1):
            TabUp[I] = fctC(tRK[I])
...
def analytics_Cauchy_1IMod0B(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt):
    Force=float(sourcef["Force"])
    f0=float(sourcef["Frequence"])
    KteV=1
    KteS=-1
...
def analytics_Cauchy_1IMod0Z(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt):
    Force=float(sourcef["Force"])
    f0=float(sourcef["Frequence"])
    KteV=1
    KteS=-1
    ...
    SolV =np.zeros(len(X))
    SolS=np.zeros(len(X))
    trace=preProcessSMM(modulation,mat,frontiere,sourcef,dt,Nt,Nt)
    for i in range(len(X)):
        if X[i] < Alpha:
            F2=sce(Tinit - (X[i] - cm[0] * (Tmax - Tinit)) / cm[0],f0)
            TempV = KteV * 1 / cm[0] * F2 * Force
...
def analytics_SourceV_1IMod0B(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt):
    Tinit=0
    Force=float(sourcef["Force"])
    f0=float(sourcef["Frequence"])
    x0=float(sourcef["Xsource"])
...
def analytics_SourceV_1IMod0Z(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt):
    Tinit=0
    Force=float(sourcef["Force"])
    f0=float(sourcef["Frequence"])
    x0=float(sourcef["Xsource"])
...
def analytics_1IMod(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt):
    Kn=float(frontiere["Kn_0"])
    Mn=float(frontiere["Mn_0"])
    namorM = "zeta_"+str(0)
    if frontiere.get(namorM)==None:
    ...
    if sourcef['Forme'] == 'Cauchy':
        if Kn != 0 and Mn==0:
            SolV,SolS=analytics_Cauchy_1IModK(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt)
        if Kn == 0 and Mn!=0:
            SolV,SolS=analytics_Cauchy_1IModM(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt)
        if Kn == 0 and Mn==0:
            if etaC!=0 and etaM==0:
                SolV,SolS=analytics_Cauchy_1IMod0B(Tmx,X,sourcef,modulation,frontiere,mat,Nt,dt,Nt)
            if etaC==0 and etaM!=0:
                SolV,SolS=analytics_Cauchy_1IMod0Z(Tmx,X,sourcef,modulation,frontiere,mat,Nt,dt,Nt)
    if sourcef['Forme'] == 'point source':
        if Kn != 0 and Mn==0:
            SolV,SolS=analytics_SourceV_1IModK(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt)
        if Kn == 0 and Mn!=0:
            SolV,SolS=analytics_SourceV_1IModM(Tmx,X,sourcef,modulation,frontiere,mat,NOde,dt,Nt)
        if Kn == 0 and Mn==0:
            if etaC!=0 and etaM==0:
                SolV,SolS=analytics_SourceV_1IMod0B(Tmx,X,sourcef,modulation,frontiere,mat,Nt,dt,Nt)
            if etaC==0 and etaM!=0:
                SolV,SolS=analytics_SourceV_1IMod0Z(Tmx,X,sourcef,modulation,frontiere,mat,Nt,dt,Nt)
    return SolV, SolS