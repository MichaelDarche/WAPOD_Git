...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
############# Fields ######################
uI=1/2/syp.I*(syp.exp(syp.I*(omega*(t-x/c0)))-syp.exp(syp.I*(-omega*(t-x/c0))))
sI=-1/2*omega*Z0*(syp.exp(syp.I*(omega*(t-x/c0)))+syp.exp(-syp.I*(omega*(t-x/c0))))
...
def somme_variables_symboliques_test(n):
    ...
    uRpn = -syp.Sum(syp.Indexed('Rp', l)*syp.exp(syp.I*(omega+l*Omega)*(t+x/c0))+syp.Indexed('rp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t+x/c0)), (l, 1, n))
    uRmn = -syp.Sum(syp.Indexed('Rm', l)*syp.exp(syp.I*(omega-l*Omega)*(t+x/c0))+syp.Indexed('rm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t+x/c0)), (l, 1, n))
    uRn=-R1*syp.exp(syp.I*(omega*(t+x/c0)))+uRpn+uRmn-r1*syp.exp(-syp.I*(omega*(t+x/c0)))
    
    sRpn = -Z0*syp.Sum((omega+l*Omega)*(syp.Indexed('Rp', l)*syp.exp(syp.I*(omega+l*Omega)*(t+x/c0))-syp.Indexed('rp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t+x/c0))), (l, 1, n))#
    sRmn = -Z0*syp.Sum((omega-l*Omega)*(syp.Indexed('Rm', l)*syp.exp(syp.I*(omega-l*Omega)*(t+x/c0))-syp.Indexed('rm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t+x/c0))), (l, 1, n))#
    sRn=-Z0*R1*omega*syp.exp(syp.I*(omega*(t+x/c0)))+sRpn+sRmn+Z0*r1*omega*syp.exp(-syp.I*(omega*(t+x/c0)))
    
    uTpn = syp.Sum(syp.Indexed('Tp', l)*syp.exp(syp.I*(omega+l*Omega)*(t-x/c1))+syp.Indexed('tp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t-x/c1)), (l, 1, n))#
    uTmn = syp.Sum(syp.Indexed('Tm', l)*syp.exp(syp.I*(omega-l*Omega)*(t-x/c1))+syp.Indexed('tm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t-x/c1)), (l, 1, n))#
    uTn= T1*syp.exp(syp.I*(omega*(t-x/c1)))+uTpn+uTmn+t1*syp.exp(-syp.I*(omega*(t-x/c1)))
    
    sTpn = -Z1*(omega+l*Omega)*syp.Sum((syp.Indexed('Tp', l)*syp.exp(syp.I*(omega+l*Omega)*(t-x/c1))-syp.Indexed('tp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t-x/c1))), (l, 1, n))#
    sTmn = -Z1*(omega-l*Omega)*syp.Sum((syp.Indexed('Tm', l)*syp.exp(syp.I*(omega-l*Omega)*(t-x/c1))-syp.Indexed('tm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t-x/c1))), (l, 1, n))#
    sTn=-Z1*omega*T1*syp.exp(syp.I*(omega*(t-x/c1)))+sTpn+sTmn+Z1*omega*t1*syp.exp(-syp.I*(omega*(t-x/c1)))
    # Simplifier l'expression
    matCoeffs=syp.zeros(4*(2*n+1),1)
    matCoeffs[4*n]=R1
    matCoeffs[4*n+1]=r1
...
param={omega:(300*2*syp.pi).evalf(),Omega:(20*2*syp.pi).evalf(),rho0:1200,rho1:1200,c0:2800,c1:2800,Delta:0.9,delta:0.9,M0:10000,K0:2.45e9}
...
for i in range(1, n+1):
    ...
    coefsTaNorm.append(syp.Abs(RTNum.args[0][4*(n-i)+3]/coefsT[0]))
    coefsTa.append(syp.Abs((RTNum.args[0][4*(n+i)+2])))  # /coefsR[0])
