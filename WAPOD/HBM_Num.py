...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 10:10:33 2024
...
def sysPsi(n,dK,rho,c,K,f,F):
    ...
    for i in range(-n,n+1):
        k=i+n
        okm1=np.abs(2*np.pi*(f+(i-1)*F))
        ok=np.abs(2*np.pi*(f+i*F))
        okp1=np.abs(2*np.pi*(f+(i+1)*F))
