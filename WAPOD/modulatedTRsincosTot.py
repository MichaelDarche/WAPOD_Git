...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
############# Fields ######################
uI=1/2*(syp.exp(syp.I*(omega*(t-x/c0)))+syp.exp(syp.I*(-omega*(t-x/c0))))
sI=-1/2*syp.I*omega*Z0*(syp.exp(syp.I*(omega*(t-x/c0)))-syp.exp(-syp.I*(omega*(t-x/c0))))
...
def somme_variables_symboliques(n):
    ...
    uRn=+uRpn+uRmn-R1*syp.exp(syp.I*(omega*(t+x/c0)))-r1*syp.exp(-syp.I*(omega*(t+x/c0)))
    ...
    uTn=+uTpn+uTmn+T1*syp.exp(syp.I*(omega*(t-x/c1)))+t1*syp.exp(-syp.I*(omega*(t-x/c1)))
...
def somme_variables_symboliques_test(n):
    # Declarer une variable symbolique i
    l = syp.symbols('l')
    ...
    sRpn = -syp.I*Z0*syp.Sum((omega+l*Omega)*(syp.Indexed('Rp', l)*syp.exp(syp.I*(omega+l*Omega)*(t+x/c0))-syp.Indexed('rp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t+x/c0))), (l, 1, n))#
    sRmn = -syp.I*Z0*syp.Sum((omega-l*Omega)*(syp.Indexed('Rm', l)*syp.exp(syp.I*(omega-l*Omega)*(t+x/c0))-syp.Indexed('rm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t+x/c0))), (l, 1, n))#
    sRn=-syp.I*Z0*R1*omega*syp.exp(syp.I*(omega*(t+x/c0)))+sRpn+sRmn+syp.I*Z0*r1*omega*syp.exp(-syp.I*(omega*(t+x/c0)))
    
    uTpn = syp.Sum(syp.Indexed('Tp', l)*syp.exp(syp.I*(omega+l*Omega)*(t-x/c1))+syp.Indexed('tp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t-x/c1)), (l, 1, n))#
    uTmn = syp.Sum(syp.Indexed('Tm', l)*syp.exp(syp.I*(omega-l*Omega)*(t-x/c1))+syp.Indexed('tm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t-x/c1)), (l, 1, n))#
    uTn= uTpn+uTmn+T1*syp.exp(syp.I*(omega*(t-x/c1)))+t1*syp.exp(-syp.I*(omega*(t-x/c1)))
    
    sTpn = -syp.I*Z1*syp.Sum((omega+l*Omega)*(syp.Indexed('Tp', l)*syp.exp(syp.I*(omega+l*Omega)*(t-x/c1))-syp.Indexed('tp', l)*syp.exp(-syp.I*(omega+l*Omega)*(t-x/c1))), (l, 1, n))#
    sTmn = -syp.I*Z1*syp.Sum((omega-l*Omega)*(syp.Indexed('Tm', l)*syp.exp(syp.I*(omega-l*Omega)*(t-x/c1))-syp.Indexed('tm', l)*syp.exp(-syp.I*(omega-l*Omega)*(t-x/c1))), (l, 1, n))#
    sTn=+sTpn+sTmn-syp.I*Z1*omega*T1*syp.exp(syp.I*(omega*(t-x/c1)))+syp.I*Z1*omega*t1*syp.exp(-syp.I*(omega*(t-x/c1)))
    # Simplifier l'expression
    matCoeffs=syp.zeros(4*(2*n+1),1)
    matCoeffs[4*n]=R1
    matCoeffs[4*n+1]=r1
...
#% n-Displacement
n=9
uRn,sRn,uTn,sTn,matCoeffs=somme_variables_symboliques_test(n)
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
vI=syp.diff(uI,t)
vRn=syp.diff(uRn,t)
vTn=syp.diff(uTn,t)
...
for i in range(2*n+1):
    ...
    eqsA.append(syp.Eq((hV2A[0]).subs(h,syp.exp(syp.I*omega*t)).subs(c1,c0).subs(rho1,rho0).simplify(),0))
    eqsA.append(syp.Eq((hS2A[0]).subs(h,syp.exp(syp.I*omega*t)).subs(c1,c0).subs(rho1,rho0).simplify(),0))
    eqsA.append(syp.Eq((hV2A[2]).subs(h,syp.exp(syp.I*omega*t)).subs(c1,c0).subs(rho1,rho0).simplify(),0))
    eqsA.append(syp.Eq((hS2A[2]).subs(h,syp.exp(syp.I*omega*t)).subs(c1,c0).subs(rho1,rho0).simplify(),0))
...
# for i in range(2*n+1):
#     hV2=syp.poly(polVCoeffs[i+1],h).all_coeffs()
#     hS2=syp.poly(polSCoeffs[i+1TijN[f'00']],h).all_coeffs()
...
# coefsRLR=[]
# coefsRLRNorm=[1,1]
# coefsRLR.append((RTRLNum.args[0][4*n]))
# coefsRLR.append((RTRLNum.args[0][4*n+1]))
# for i in range(1,n+1):
#     coefsRLR.append((RTRLNum.args[0][2*(n-i)]))#/coefsR[0])
#     coefsRLR.append((RTRLNum.args[0][2*(n-i)+1]))
#     coefsRLRNorm.append((RTRLNum.args[0][2*(n-i)])/coefsRLR[0])
#     coefsRLRNorm.append((RTRLNum.args[0][2*(n-i)+1])/coefsRLR[0])
#     coefsRLR.append((RTRLNum.args[0][2*(n+i)]))#/coefsR[0])
...
#     VTnORL=VTnORL.subs(syp.Indexed('Tm', j+1),syp.exp(syp.I*(omega-j*Omega)/c0*(x-x0))*coefsRLT[4*j+2]).subs(syp.Indexed('Tp', j+1),syp.exp(syp.I*(omega+j*Omega)/c0*(x-x0))*coefsRLT[4*j+4])
...
param={omega:(91*2*syp.pi).evalf(),Omega:(9*2*syp.pi).evalf(),rho0:1200,rho1:1200,c0:2800,c1:2800,Delta:0.9,delta:0.9,M0:10000,K0:2.45e9}
...
for i in range(2*n+1):    
    hV2=syp.poly(polVCoeffs[i+1],h).all_coeffs()
    hS2=syp.poly(polSCoeffs[i+1],h).all_coeffs()
    eqsNum.append(syp.Eq((hV2[0]).subs(param).simplify(),0))
    eqsNum.append(syp.Eq((hS2[0]).subs(param).simplify(),0))
    eqsNum.append(syp.Eq((hV2[2]).subs(param).simplify(),0))
    eqsNum.append(syp.Eq((hS2[2]).subs(c1,c0).subs(param).simplify(),0))
...
coefsR=[]
coefsRNorm=[1,1]
coefsR.append((RTNum.args[0][4*n]))
coefsR.append(syp.Abs(RTNum.args[0][4*n+1]))
for i in range(1,n+1):
    coefsR.append((RTNum.args[0][4*(n-i)+2]))#/coefsR[0])
    coefsR.append(syp.Abs(RTNum.args[0][4*(n-i)+1]))
    coefsRNorm.append((RTNum.args[0][4*(n-i)])/coefsR[0])
    coefsRNorm.append(syp.Abs(RTNum.args[0][4*(n-i)+1])/coefsR[0])
    coefsR.append((RTNum.args[0][4*(n+i)]))#/coefsR[0])
...
coefsT=[]
coefsTNorm=[1.,1.]
coefsT.append(RTNum.args[0][4*n+2])
coefsT.append(RTNum.args[0][4*n+3])
for i in range(1,n+1):
    coefsT.append(RTNum.args[0][4*(n-i)+2])#/coefsR[0])
    coefsT.append((RTNum.args[0][4*(n-i)+3]))
    coefsTNorm.append((RTNum.args[0][4*(n-i)+2])/coefsT[0])
...
coefsTr=[]
coefsTrNorm=[1.,1.]
coefsTr.append(syp.re(RTNum.args[0][4*n+2]))
coefsTr.append(syp.re(RTNum.args[0][4*n+3]))
for i in range(1,n+1):
    coefsTr.append(syp.re(RTNum.args[0][4*(n-i)+2]))#/coefsR[0])
    coefsTr.append(syp.re(RTNum.args[0][4*(n-i)+3]))
    coefsTrNorm.append(syp.re((RTNum.args[0][4*(n-i)+2])/coefsTr[0]))
    coefsTrNorm.append(syp.re(RTNum.args[0][4*(n-i)+3]/coefsTr[0]))
    coefsTr.append(syp.re((RTNum.args[0][4*(n+i)+2])))#/coefsR[0])
    coefsTr.append(syp.re(RTNum.args[0][4*(n+i)+3]))#/coefsR[0])
    coefsTrNorm.append(syp.re((RTNum.args[0][4*(n+i)+2])/coefsTr[0]))
    coefsTrNorm.append(syp.re(RTNum.args[0][4*(n+i)+3]/coefsTr[0]))
coefsTi=[]
coefsTiNorm=[1.,1.]
coefsTi.append(syp.im(RTNum.args[0][4*n+2]))
coefsTi.append(syp.im(RTNum.args[0][4*n+3]))
for i in range(1,n+1):
    coefsTi.append(syp.im(RTNum.args[0][4*(n-i)+2]))#/coefsR[0])
    coefsTi.append(syp.im(RTNum.args[0][4*(n-i)+3]))
    coefsTiNorm.append(syp.im((RTNum.args[0][4*(n-i)+2])/coefsT[0]))
    coefsTiNorm.append(syp.im(RTNum.args[0][4*(n-i)+3]/coefsT[0]))
    coefsTi.append(syp.im((RTNum.args[0][4*(n+i)+2])))#/coefsR[0])
    coefsTi.append(syp.im(RTNum.args[0][4*(n+i)+3]))#/coefsR[0])
    coefsTiNorm.append(syp.im((RTNum.args[0][4*(n+i)+2])/coefsT[0]))
    coefsTiNorm.append(syp.im(RTNum.args[0][4*(n+i)+3]/coefsT[0]))
coefsTa = []
coefsTaNorm = [1.,1.]
coefsTa.append(syp.Abs(RTNum.args[0][4*n+2]))
coefsTa.append(syp.Abs(RTNum.args[0][4*n+3]))
for i in range(1, n+1):
    coefsTa.append(syp.Abs(RTNum.args[0][4*(n-i)+2]))  # /coefsR[0])
    coefsTa.append(syp.Abs(RTNum.args[0][4*(n-i)+3]))
    coefsTaNorm.append(syp.Abs((RTNum.args[0][4*(n-i)+2])/coefsTa[0]))
    coefsTaNorm.append(syp.Abs(RTNum.args[0][4*(n-i)+3]/coefsTa[0]))
    coefsTa.append(syp.Abs((RTNum.args[0][4*(n+i)+2])))  # /coefsR[0])
    coefsTa.append(syp.Abs(RTNum.args[0][4*(n+i)+3]))#/coefsR[0])
    coefsTaNorm.append(syp.Abs((RTNum.args[0][4*(n+i)+2])/coefsTa[0]))
    coefsTaNorm.append(syp.Abs(RTNum.args[0][4*(n+i)+3]/coefsTa[0]))
...
for j in range(n):
    #VTnO=VTnO.subs(syp.Indexed('Tm', j+1),coefsT[4*j+1]).subs(syp.Indexed('Tp', j+1),coefsT[4*j+2])
