...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_SpaceModulation_PS_Bloch/")
...
def updateComp(frame):
    ...
    for i in range(alpha.size-1):
        if modul.fonctionModMP(pFilm*dt2p*frame,alpha[i],frontiere,i)<0.:
            plt.axvline(x=alpha[i],color='coral')#,linestyle='--')
        elif modul.fonctionModMP(pFilm*dt2p*frame,alpha[i],frontiere,i)>0.:
            plt.axvline(x=alpha[i],color='purple')#,linestyle='--')
        else:
...
plt.tight_layout()
namefig="Figures/PRSA_U_MK_"+sourcef["Frequence"]+"_"+modulation["Freq"]+".eps"
...
# plt.plot(VfilmH[500,:])
# plt.title("$nt=$500")
namefig="Figures/PRSA_V_MK_"+sourcef["Frequence"]+"_"+modulation["Freq"]+".eps"
plt.savefig(str(namefig), format='eps')
...
plt.legend(fontsize=16)#loc="lower right",
plt.tight_layout()
namefig="Figures/PRSAzoom_U_MK_"+sourcef["Frequence"]+"_"+modulation["Freq"]+".eps"
plt.savefig(str(namefig), format='eps')
...
# plt.plot(VfilmH[500,:])
# plt.title("$nt=$500")
namefig="Figures/PRSAzoom_V_MK_"+sourcef["Frequence"]+"_"+modulation["Freq"]+".eps"
plt.savefig(str(namefig), format='eps')
...
import funcPWE
fctModE="np.heavyside"
fctModR="np.heavyside"
NE=8
# if tau!=0:
#     omegavect,VP=funcPWE.fillMatrix(tau,Eef0,rhoef0,EpsE,EpsR,fctModE,fctModR,NE,N=16)
#     kmax=max(np.real(VP[NE-1,:])/2/np.pi)
k,VP=funcPWE.fillMatrixS(10,valE,valR,deltaE,deltaR,fctModE,fctModR,NE,Nk=50,N=16,Ni=500)
ffg.dispFK(Ufilm[::,::],dt,dx)
plt.figure()
    # for i in range(NE):
