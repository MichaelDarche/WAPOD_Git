...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_dpha/")
...
plt.figure()
plt.pcolor(wavv,frqv,np.log(FFK),vmin=0, vmax = 4)
plt.ylim([0,200])
plt.xlim([-np.pi/15,np.pi/15])
plt.colorbar()
plt.xlabel("k (1/m)",fontsize=20)
...
plt.ylim([0,200])
plt.xlim([-np.pi/10,0])
plt.xlabel(r'$k$ (1/m)',fontsize=20)
