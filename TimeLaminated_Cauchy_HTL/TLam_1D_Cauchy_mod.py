...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/TimeLaminated_Cauchy_HTL/")
...
#% Initial conditions
U0=scheme1D.initCauchyProblemHTL(configuration,frontiere,mat,sourcef,modulation)
...
import time
time0=time.time()
Res,Vfilm,Sfilm=scheme1D.FD_CauchyVIIHomoGS_Period_HTL(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,configuration,mat,modulation,pFilm)
...
maxV=np.max([np.max(Vfilm[:800]),-np.min(Vfilm[:800])])
print(timeM)  
...
# Affiche l'animation
plt.show()
# nomAnim="Film_R_"+sourcef["Frequence"]+"Hz_"+modulation["Freq"]+"Hz.mp4"
...
plt.ylabel(r"$\mathcal{E}$ (J)",fontsize=20)
plt.tight_layout()

nomEner="Energy_R_"+sourcef["Frequence"]+"Hz_"+modulation["Freq"]+"Hz.png"
plt.savefig(nomEner, format='png')

plt.figure()
plt.plot(tp[:-1200],energyV[:-1199])
plt.imshow([rhoT[:-1199:pFilm]], cmap='gray', extent=[tp[0], tp[-1200], 0,2], aspect='auto', alpha=0.35)
# plt.title("Energy")
plt.xlabel(r"$t$ (s)",fontsize=20)
plt.ylabel(r"$\mathcal{E}$ (J)",fontsize=20)
plt.tight_layout()

nomEner="Energy_zoom_"+sourcef["Frequence"]+"Hz_"+modulation["Freq"]+"Hz.png"
plt.savefig(nomEner, format='png')
#############DISPERSION###########
...
plt.axvline(x=k1*c_r*tau,color='red',linestyle='--',linewidth=2)
plt.axvline(x=k2*c_r*tau,color='blue',linestyle='--',linewidth=2)
plt.xlabel(r"$k c\tau$",fontsize=20)
plt.ylabel(r"$\omega\tau$",fontsize=20)
nomDisp="Disp_R_"+sourcef["Frequence"]+"Hz_"+modulation["Freq"]+"Hz.png"
...
# plt.title("LW : $t=$"+tmax)
# plt.axvline(x=alpha[0],color='gray',linestyle='--')
# # plt.savefig('Figures/LW-S_300.eps', fo+"Hz_"+modulation["Freq"]+"Hz.mp4"
#animation.save(nomAnim, writer='ffmpeg', fps=100)rmat='eps')
...
# # Affiche l'animation
# plt.show()+"Hz_"+modulation["Freq"]+"Hz.mp4"
