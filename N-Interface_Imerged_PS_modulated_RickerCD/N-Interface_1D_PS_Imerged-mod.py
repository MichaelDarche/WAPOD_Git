...
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_RickerCD/")
...
plt.figure()
plt.plot(t[:-1],energyM)
plt.plot(t[:-1],energyV)
plt.plot(t[:-1],energyI)
plt.imshow([modula], cmap="gray",extent=[t[0], t[-2], 0,np.max(energyM)], aspect='auto', alpha=0.35)
...
plt.figure()
plt.pcolor(wavv[::],frqv[::],np.log(FFK[::,::]))#,vmin=-5, vmax=5)
#plt.ylim([0,200])
