...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_FastS2/")
...
matFSW=inputReading.readfile('MilieuFSW.txt')
frontiereFSW=inputReading.readfile('FrontiereFSW.txt')  
################ Parameters of the simulation   
Nmat,rhoW,cmW,S1W=inputReading.materialWillis(matFSW)
alphaFSW=inputReading.frontiere(frontiereFSW)
rhoxFSW,CelxFSW,S1xFSW=inputReading.affectMaterialsWillis(alphaFSW,rhoW,cmW,S1W,xmin,xmax,Nx,dx)


cmaxw=0
for i in range(Nmat):
    cwil=max(abs(inputReading.c_Willis(rhoW[i],cmW[i],S1W[i])))
    cmaxw=max(cmaxw,cwil)
dtNR=scheme1D.timeStep(CFL,dx,cmaxw)


dtmin=min(dt2v,dt,dtNR)
Tfin=0.3

Ntt=int(np.ceil(Tfin/dtmin))
Tmax=Ntt*dtmin
tmax=str(Tmax)+"s"
K=4
...
import time
time0=time.time()
U,dt2=scheme1DH.temporalSchemePS0MKP(Ntt,dtmin,configuration,sourcef,nb_frontieres,frontiere,mat)
Res,Vfilm,Sfilm,CSVM=scheme1D.FD_sourceVII(U0,Ntt,Nx,K,rhox,Celx,dt2,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm,rCSVM="yes")#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
timef=time.time()
coeffsce=rhox[int(x_0/dx)]/rhoxFSW[int(x_0/dx)]
ResFS2,VfilmFS2,SfilmFS2=scheme1D.FD_sourceVWillis(U0,Ntt,Nx,K,rhoxFSW,CelxFSW,S1xFSW,dt2,dx,x_0,coeffsce*sce,1)
maxV=np.max([np.max(Vfilm),-np.min(Vfilm)])
timeN_mod=timef-time0
print(timeN_mod)  

# nbcell, nb_milieux, blocs =generatePeriodic.lire_fichier_milieu("MilieuPeriodiqueFS.txt")
# nbcell, nb_frontieres, blocsF =generatePeriodic.lire_fichier_frontieres("FrontieresPeriodiqueFS.txt")

# generatePeriodic.ecrire_milieu("MilieuFS.txt", nbcell, nb_milieux, blocs)
# generatePeriodic.ecrire_frontieres_sortie("FrontiereFS.txt", nbcell, nb_frontieres, blocsF)

################# Input files 
#%
# matFS=inputReading.readfile('MilieuFS.txt')
# frontiereFS=inputReading.readfile('FrontiereFS.txt')  
# ################ Parameters of the simulation   
# NmatFS,rhoFS,cmFS=inputReading.material(matFS)
# alphaFS=inputReading.frontiere(frontiereFS)
# rhoFS,CelFS=inputReading.affectMaterials(alphaFS,rhoFS,cmFS,xmin,xmax,Nx,dx)
# pFilmFS=2

# sceFS=np.zeros(2*Ntp+1)
# tpFS=np.linspace(0,Tmax,2*Ntp+1)
# for i in range(2*Ntp+1):
#     sceFS[i]=source.choice_timefct(sourcef,tpFS[i])


# coeffsce=rhox[int(x_0/dx)]/rhoxFSW[int(x_0/dx)]

# ResFS,VfilmFS,SfilmFS=scheme1D.FD_sourceVII(U0,2*Ntp,Nx,K,rhoFS,CelFS,dt2p/2,dx,x_0,coeffsce*sceFS,configuration,matFS,frontiereFS,modulation,pFilmFS,rCSVM="no")#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
...
Ufilm=ffg.VtoU(Vfilm,dt2)
# UfilmFS=ffg.VtoU(VfilmFS,dt2p)
UfilmFS2=ffg.VtoU(VfilmFS2,dt2)
V=ffg.UtoV(U,dt2)
S=ffg.UtoS(U,dx)
maxU=np.max(Ufilm)
...
Poynting=np.zeros([Nt,Nx])
Poynting0=np.zeros([Nt,Nx])
PoyntingFS=np.zeros([Nt,Nx])

for indT in range(Ntt):
    Poynting[indT,:]=Vfilm[indT,:]*Sfilm[indT,:]
    Poynting0[indT,:]=V[indT,:]*S[indT,:]
    PoyntingFS[indT,:]=VfilmFS2[indT,:]*SfilmFS2[indT,:]
    
    
Ntfig=int(np.floor(0.2/dt2))
maxUt=np.max([np.max(Ufilm[Ntfig,:]),-np.min(Ufilm[Ntfig,:])])    


plt.figure()
plt.plot(X,Poynting[Ntfig,:])
plt.plot(X,PoyntingFS[Ntfig,:])
...
for i in range(alpha.size-1):
    if frontiere["Contact_"+str(i)] != "parfait":
        v = modul.fonctionModMP(Ntfig*dt2, alpha[i], frontiere, i)
        vals.append(v)
    else:
        vals.append(0)
vals = np.array(vals)
...
def updateComp(frame):
    ...
    for i in range(alpha.size-1):
        if frontiere["Contact_"+str(i)] != "parfait":
            v = modul.fonctionModMP(frame*dt2, alpha[i], frontiere, i)
            vals.append(v)
        else:
            vals.append(0)
    vals = np.array(vals)
    ...
    sm.set_array([])
    plt.scatter(X[::8],Ufilm[8*frame,::8],marker=r'$\bigcirc$',label=r'$U_h$',color='red',s=30)
    # plt.plot(X[::1],Ufilm[Ntfig,::1],linestyle=None,marker='o',color='white',markersize=2)
    plt.plot(X[::8],U[8*frame,::8],linewidth=3,label=r'$U_0$')
    # plt.plot(X,UfilmFS[frame,:],':',linewidth=3,label=r'$U_{fs}$',color='limegreen')
    plt.plot(X[::8],UfilmFS2[8*frame,::8],':',linewidth=3,label=r'$U_{fs}$',color='k')
    ...
    plt.ylim([-maxUt,maxUt])
    plt.tight_layout()
    plt.legend(fontsize=14)


total_frames = int(Vfilm.shape[0]/8)
fig, ax = plt.subplots()

# Cree l'animation
animationComp = FuncAnimation(fig, updateComp, frames=total_frames, interval=1)
plt.show()
# nomAnim="Film-MicroHFS-1I_Raph_"+sourcef["Frequence"]+frontiere["Freq_0"]+"Hz.mp4"
...
def updateComp(frame):
    ...
    #plt.plot(X,VfilmFS[frame,:],':',linewidth=3,label=r'$U_{fs}$',color='limegreen')
    # plt.plot(X, Ufilm[frame,:],'*')#, label=f'Frame {frame}')
    # plt.plot(X, U[frame,:],'-')#, label=f'Frame {frame}')
    # plt.plot(X, VfilmGS[frame,:],"-")#, label=f'Frame {frame}')
    # plt.plot(X, VfilmSS[frame,:],"-")#, label=f'Frame {frame}')
...
modulK=Clist[0]*(1+modula)
# dmodula=np.zeros(int(Ntfig/pFilm))

energyV=np.zeros(int(Ntt/pFilm))
energyI=np.zeros(int(Ntt/pFilm))
energyhomo=np.zeros(int(Ntt/pFilm))

for ti in range(int(Ntt/pFilm)):
    energyV[ti]=dx/2*np.sum(rhox*Vfilm[ti]*Vfilm[ti])+dx/2*np.sum(1/rhox/Celx/Celx*Sfilm[ti]*Sfilm[ti])
    energyhomo[ti]=dx/2*np.sum(rhoeff(ti*dt2)*V[ti]*V[ti])+dx/2*np.sum(rhoeff(ti*dt2)*Ceff(ti*dt2)*Ceff(ti*dt2)*S[ti]*S[ti])
...
Ntsismo=1800
maxVt=0.5*np.max([np.max(Vfilm[:Ntsismo,:]),-np.min(Vfilm[:Ntsismo,:])])    
ffg.sismoNorm(X, tp, V,10,pFilm,Nt=Ntsismo,norm=maxVt)
ffg.sismoNorm(X, tp, VfilmFS2,10,pFilm,Nt=Ntsismo,norm=maxVt)


Ntfig=int(np.floor(0.3/dt2))
...
plt.scatter(X[::1],Ufilm[Ntfig,::1],marker=r'$\bigcirc$',label=r'$U_h$',color='red',s=40)
plt.plot(X,U[Ntfig,:],linestyle='None',marker='o',markersize=3,label=r'$U_0$')
# plt.plot(X,UfilmFS2[Ntfig,:],color='black',linewidth=3,label=r'$U_{fs}$')
...
plt.plot(X,UfilmFS2[Ntfig,:],linewidth=3,label=r'$U_{fs}$')
...
plt.plot(X,VfilmFS2[Ntfig,:],linewidth=3,label=r'$U_{fs}$')
# plt.axvline(x=alpha[0],color='black')
plt.xlabel(r'$X$ (m)',fontsize=16)
...
if tau!=0:
    ...
    ffg.dispFK_tperiodic2(Ufilm[:Ntfig,:],dt2,dx,fmlist[0],kmax/2)
    for i in range(NE):
