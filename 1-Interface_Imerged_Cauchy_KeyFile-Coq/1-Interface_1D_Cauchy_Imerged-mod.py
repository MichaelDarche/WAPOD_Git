...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-Interface_Imerged_Cauchy_KeyFile-Coq")
...
sys.path.append("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/WAPOD/")
import scheme1D_TDold as scheme1D
import source
import inputReading

import WavToTXT as wttxt
...
frontiereR=inputReading.readfile('FrontiereR.txt')     
...
modulationROW=inputReading.readfile('ModulationROW.txt')
...
# vectTime,data,sR,sW,nsamp=wttxt.wave2np("Coq")

vectTime=np.arange(0,0.025,1/44100)
sR=44100
sW=2
nsamp=vectTime.size
data=np.zeros(np.size(vectTime))
for i in range(len(vectTime)):
    data[i]=source.jkps_t(vectTime[i],150)

SceComp=wttxt.interpol(vectTime,data,dt)

plt.figure()
plt.plot(vectTime,data)
#% Initial conditions
...
def cosine_taper(signal, alpha=0.025,zp=18):
    zeropadding=2**zp
    Size=signal.shape[0]
...
def cosine_taper0(signal, alpha=0.02):
    Size=signal.shape[0]
    M=np.floor((Size*alpha)/2+0.5);
...
import time
time0=time.time()
Res,Vfilm,Sfilm=scheme1D.FD_sourceVIIOneWay(U0,Nt,Nx,K,rhox,Celx,dt,dx,x_0,sigcor,configuration,mat,frontiere,modulation,pFilm)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
timeM=time.time()-time0
maxV=np.max([np.max(Vfilm),-np.min(Vfilm)])
print(timeM)  

# Vmodul=wttxt.interpol(t, Vfilm[:,Nxr], 1/sR)
# intV=(2*Vmodul*cm[0]).astype(dtype=np.int16)
# modulatedSound=wttxt.np2wav("CoqModulated",intV,sR,sW,nsamp-1)

import time
time0=time.time()
ResR,VfilmR,SfilmR=scheme1D.FD_sourceVIIKeyOW(U0,Nt-1,Nx,K,rhox,Celx,dt,dx,x_r,np.flip(Vfilm[:,Nxr])*cm[0],configuration,mat,frontiereR,modulationROW,pFilm)
#ResR1,VfilmR1,SfilmR1=scheme1D.FD_sourceVIIKey(U0,Nt-1,Nx,K,rhox,Celx,-dt,dx,x_r,-2*np.flip(Vfilm[:,Nxr])*cm[0],configuration,mat,frontiere,modulationR,pFilm)
# ResR,VfilmR,SfilmR=scheme1D.FD_sourceVIIKey(Res,Nt,Nx,K,rhox,Celx,-dt,dx,configuration,mat,frontiere,modulationR,pFilm,rCSVM="no")
#U0,Nt,Nx,K,rhox,Celx,dt,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)#:(U0,Nt,Nx,K,rhox,Celx,dt,dx,"no")
timeM=time.time()-time0
maxV=np.max([np.max(Vfilm),-np.min(Vfilm)])
print(timeM)  

# Vback=wttxt.interpol(t, np.flip(VfilmR[:,int(x_0/dx)]), 1/sR)
# intR=(2*Vback*cm[0]).astype(dtype=np.int16)
# modulatedRSound=wttxt.np2wav("CoqBack",intR,sR,sW,nsamp-1)

plt.figure()
plt.plot(Vfilm[:-1,int(x_0/dx+2)]*2800)
#plt.plot(np.flip(VfilmR[:,int(x_0/dx)]))
plt.plot(np.flip(VfilmR[:,int(x_0/dx+2)])*2800)
# plt.figure()
plt.plot(Vfilm[:-1,Nxr-2]*2800)
plt.plot(np.flip(VfilmR[:,Nxr-2])*2800)


sigcor = cosine_taper0(SceComp)
fftSigMod=1/Nt*np.fft.fftshift(np.fft.fft(2*cm[0]*Vfilm[:-1,Nxr],n=N))
fftSigBack=1/Nt*np.fft.fftshift(np.fft.fft(2*cm[0]*VfilmR[:,int(x_0/dx)],n=N))
plt.figure()
plt.plot(np.abs(freq),np.abs(fftSig),linewidth=3)
plt.plot(np.abs(freq),np.abs(fftSigMod),linewidth=3)
plt.plot(np.abs(freq),np.abs(fftSigBack),linewidth=3)
# plt.xlim([0,300])
# plt.ylim([0,0.25])
plt.xlabel(r'$f$ (Hz)',fontsize=16)
...
plt.imshow([1+modula], cmap='gray', extent=[0, t[-1], 0,max(energyM)*1.1], aspect='auto', alpha=0.35)
