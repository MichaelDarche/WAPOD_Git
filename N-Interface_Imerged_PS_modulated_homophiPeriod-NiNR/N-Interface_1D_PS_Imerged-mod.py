...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_homophiPeriod-NiNR/")
...
nbcell, nb_milieux, blocs =generatePeriodic.lire_fichier_milieu("MilieuPeriodique2.txt")
...
def updateComp(frame):
    ...
    for i in range(alpha.size-1):
        if frontiere["Contact_"+str(i)] != "parfait":
            v = modul.fonctionModMP(frame*pfilm*dt2p, alpha[i], frontiere, i)
            vals.append(v)
        else:
            vals.append(0)
    vals = np.array(vals)
    ...
    sm.set_array([])
    plt.scatter(X[::2],Ufilm[frame*pfilm,::2],marker='o',label=r'$U_h$',color='red',zorder=1000,s=30)
    ...
    # plt.plot(X, Ufilm[frame*pfilm,:],'*')#, label=f'Frame {frame}')
    # plt.plot(X, VfilmGS[frame,:],"-")#, label=f'Frame {frame}')
#    plt.plot(X, VfilmSS[frame,:],"-",color="red")#, label=f'Frame {frame}')
...
total_frames = int(Vfilm.shape[0]/pfilm)
fig, ax = plt.subplots()
...
# sm.set_clim(-1.0, 1.0)
# cbar.set_label(label=r'$\varepsilon_{M,\ell}~\sin (\Omega_{M,\ell} T+\Phi_\ell)$',size=16)
        # else:
            #plt.axvline(x=alpha[i],color='lavender')
# plt.axvspan(440, 560, facecolor='g', alpha=0.3)
# for i in range(alpha.size):
#     plt.axvline(x=alpha[i],color='gray',linewidth=0.75)
...
plt.tight_layout()
# namefig="Figures/PRSA_U_MK-Ni8_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
# plt.savefig(str(namefig), format='pdf')
# namefig="Figures/PRSA_U_MK-Ni8_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
...
# plt.plot(VfilmH[500,:])
# plt.title("$nt=$500")
# namefig="Figures/PRSA_V_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
# plt.savefig(str(namefig), format='pdf')
# namefig="Figures/PRSA_V_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
# plt.savefig(str(namefig), format='png')
...
for i in range(alpha.size):
    if frontiere["Contact_"+str(i)]!="parfait":
        plt.axvline(x=alpha[i],ymin=0,ymax=1,color=cmap(vals[i]),alpha=np.abs(vals[i]/1),linewidth=0.8)#,linestyle='--')
        # elif modul.fonctionModMP(Ntfig*dt2p,alpha[i],frontiere,i)>0.:
        #     plt.axvline(x=alpha[i],ymin=0,ymax=1,color='orchid',alpha=modul.fonctionModMP(Ntfig*dt2p,alpha[i]/2,frontiere,i))
        #plt.colorbar()#,linestyle='--')
sm = mpl.cm.ScalarMappable(cmap=cmap, norm=norm)
...
sm.set_clim(-1.0, 1.0)
plt.show()
plt.scatter(X[::1],Ufilm[Ntfig,::1],marker='o',label=r'$U_h$',color='red',s=30,zorder=1000)
# plt.axvline(x=470,linestyle=':',linewidth=3,color='#1f77b4',zorder=1001)
# plt.axvline(x=500,linestyle=':',linewidth=3,color='black',zorder=1001)
# plt.axvline(x=530,linestyle=':',linewidth=3,color='#ff7f0e',zorder=1002)
y0 = plt.ylim()[0]
...
plt.legend(fontsize=16)#loc="lower right",
plt.tight_layout()
# namefig="Figures/PRSAzoom_U_MK-Ni8_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
# plt.savefig(str(namefig), format='pdf')
# namefig="Figures/PRSAzoom_U_MK-Ni8_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
# plt.savefig(str(namefig), format='png')
...
# plt.plot(VfilmH[500,:])
# plt.title("$nt=$500")
# namefig="Figures/PRSAzoom_V_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
# plt.savefig(str(namefig), format='pdf')
# namefig="Figures/PRSAzoom_V_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
# plt.savefig(str(namefig), format='png')
...
fctModR="np.sin"
NE=8
ffg.dispFK(Ufilm[:Ntfig,:],dt2p,dx)
# if np.sum(taulist)!=0:
