...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 19 19:08:47 2025
...
sourcef=inputReading.readfile('Source0.txt')   
modulation=inputReading.readfile('Modulation0.txt')
...
plt.figure()
plt.plot(tp[:-1],Ufilm[:,int(Nx//2)],linewidth=3)
plt.plot(tp,U[:,int(Nx//2)],'--',linewidth=3)
plt.plot(tp[:],Uz[:,int(Nx//2)],':',linewidth=3)
...
ffg.dispFK_tperiodic(Ufilm,delt,dx,fm,kmax)
ffg.dispFK_tperiodic(Uz,delt,dx,fm,kmax)

plt.figure()
plt.plot(Ufilm[:,int(Nx//2)])