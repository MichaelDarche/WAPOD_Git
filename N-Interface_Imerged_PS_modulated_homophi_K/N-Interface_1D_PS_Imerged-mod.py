...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_homophi_K/")
...
time0H=time.time()
ResSS2,VfilmSS2,SfilmSS2=scheme1D.FD_sourceVIIHomoSS2(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,modulation,pFilm)
timefH=time.time()
timeNH_mod=timefH-time0H
print(timeNH_mod)  
# # # ###################### NO MODULATION #################################
# time0=time.time()
...
#    plt.plot(X, VfilmSS[frame,:],"-")#, label=f'Frame {frame}')
    plt.plot(X, VfilmSS2[frame,:],"-")#, label=f'Frame {frame}')
    # plt.plot(X, VfilmSS[frame,:],"-")#, label=f'Frame {frame}')
...
ffg.sismo(X, t, VfilmSS2,20,pFilm,Nt=1000)

Ntfig=int(np.floor(0.18/dt2p))
...
plt.scatter(X[::1],Vfilm[Ntfig,::1],marker=r'$\bigcirc$',label=r'$V_h$',color='red',s=60)
plt.plot(X[::1],Vfilm[Ntfig,::1],linestyle='None',marker='o',color='white',markersize=4)
plt.plot(X,VfilmSS2[Ntfig,:],linewidth=3,label=r'$V_0$')
# plt.axvline(x=alpha[0],color='black')
plt.xlabel(r'$x$ (m)',fontsize=16)
plt.ylabel(r'$v$ (m/s)',fontsize=16)
plt.ylim([-1.25*maxVt,1.25*maxVt])
...
# plt.plot(VfilmH[500,:])
# plt.title("$nt=$500")
namefig="Figures/WP500_sin1_K_N8000_"+sourcef["Frequence"]+"_"+modulation["Freq"]+".eps"
plt.savefig(str(namefig), format='eps')


import figures
UfromV= figures.VtoU(Vfilm,dt2p)
USS2= figures.VtoU(VfilmSS2,dt2p)

plt.figure()
for i in range(alpha.size):
    plt.axvline(x=alpha[i],color='gray',linewidth=0.75)
plt.scatter(X[::1],UfromV[Ntfig,::1],marker=r'$\bigcirc$',label=r'$U_h$',color='red',s=60)
plt.plot(X[::1],UfromV[Ntfig,::1],linestyle='None',marker='o',color='white',markersize=4)
plt.plot(X,USS2[Ntfig,:],linewidth=3,label=r'$U_0$')
# plt.axvline(x=alpha[0],color='black')
plt.xlabel(r'$x$ (m)',fontsize=16)
plt.ylabel(r'$v$ (m/s)',fontsize=16)
# plt.ylim([-1.25*maxVt,1.25*maxVt])
