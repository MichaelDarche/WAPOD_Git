...
import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/N-Interface_Imerged_PS_modulated_homophiPeriod-Ni1/")
...
ResSS,VfilmSS,SfilmSS=scheme1D.FD_sourceVIIHomoSSPdA(U0,Ntp,Nx,K,rhox,Celx,dt2p,dx,x_0,sce,configuration,mat,frontiere,xmin,nb_frontieres,pFilm)
...
# # Cree l'animationen certains points
...
def updateComp(frame):
    ...
    for i in range(alpha.size-1):
        if modul.fonctionModMP(pFilm*dt2p*frame,alpha[i],frontiere,i)<0.:
            plt.axvline(x=alpha[i],color='palegreen')#,linestyle='--')
        elif modul.fonctionModMP(pFilm*dt2p*frame,alpha[i],frontiere,i)>0.:
            plt.axvline(x=alpha[i],color='plum')#,linestyle='--')
        else:
...
plt.plot(X,UfilmSS[Ntfig,:],color='red',linestyle='None',marker='o',markersize=3,label=r'$U_0$')
...
plt.tight_layout()
namefig="Figures/PRSA_U_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
plt.savefig(str(namefig), format='pdf')
namefig="Figures/PRSA_U_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
...
plt.legend(fontsize=16)#loc="lower right",
plt.tight_layout()
namefig="Figures/PRSAzoom_U_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".pdf"
plt.savefig(str(namefig), format='pdf')
namefig="Figures/PRSAzoom_U_MK-Ni2rand_"+sourcef["Frequence"]+"_"+frontiere["Freq_1"]+".png"
plt.savefig(str(namefig), format='png')
