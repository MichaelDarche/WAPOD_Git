...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 17:26:17 2023

@author: michael
"""

import os
os.chdir("/home/michael/Documents/ModulatedMetamaterials/WAPRO_TD/1-SMI_CauchyError/")
...
###### Mise en oeuvre
    ...
    df=Kf*fs/NFourier
    for pt in range(Nx):
        # THV0y[pt],THS0y[pt]=analytics.analytics1InterfaceCauchy(tshift,X[pt],sourcef,frontiere,NFourier,mat,Kf)
        THVTMy[pt],THSTMy[pt]=analytics.analytics1InterfaceCauchy(tshift+Tmax,X[pt],sourcef,frontiere,NFourier,mat,Kf)
    for i in range(2):
        K=2+2*i
        # Res=scheme1D.FD_cauchy(U0,Nt,Nx,K,rhox,Celx,dt,dx,configuration,mat,frontiere,"no")
        # # plt.figure()
        # # plt.plot(X,Res[0,:])
        # # plt.plot(X,THVTMy,'--')
        # # plt.xlabel("$x$ (m)")
        ...
        ResIm=scheme1D.FD_cauchyII(U0,Nt,Nx,K,rhox,Celx,dt,dx,configuration,mat,frontiere,"no")
        # plt.figure()
        # plt.plot(X,Res[0,:])
        # plt.plot(X,THVTMy,'--')
        # plt.xlabel("$x$ (m)")
...
plt.figure()
# plt.loglog(data[1,:],errIm[0,:],'*--',label="Law-Wendroff")
